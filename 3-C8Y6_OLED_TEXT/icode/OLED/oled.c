...
void OLED_printf(unsigned char line,unsigned char world,unsigned char size,const char *fmt, ...)
{
	...
	/* 指定x的位置 */
	if((world > 0) && (world < 16))  world = world * 8;
	else			world = 0;
	
	sprintf(temp_str,"%s", buf);
	
	/* 指定y的位置，并限制16号大小时的行数 */
	if(size == 16)  OLED_ShowString_16(world,line,(uint8_t *)temp_str,size);
	else			OLED_ShowString(world,line,(uint8_t *)temp_str,size);
